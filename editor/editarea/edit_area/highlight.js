...
EditArea.prototype.maj_highlight = function (infos) {
	...
	if (t.reload_highlight === true) {
		...
		var changes = t.checkTextEvolution(t.last_text_to_highlight, textToHighlight);
	...
	var tps_end_opti = new Date().getTime();
		...
		try {
			...
			if (newHtml.indexOf('<span') == -1 && newHtml.indexOf('</span') == -1
				...
				// get the last opened span
					...
					var nbClosed = beginStr.substr(Math.max(0, beginStr.lastIndexOf('<span', maxStartOffset - 1))).split('</span').length - 1;
					while (nbClosed > 0) {
						nbClosed--;
						parentSpan = parentSpan.parentNode;
					}
				...
				var lastEndPos, nbSubSpanBefore;
