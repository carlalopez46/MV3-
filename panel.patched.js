...
function play() {
    ...
    if (panelState.isRecording) {
        console.log("[Panel] Ignoring play request - currently recording");
        return;
    }
...
function stop() {
    ...
    if (!panelState.isPlaying && !panelState.isRecording) {
        console.log("[Panel] Ignoring stop request - already idle");
        return;
    }
    sendCommand("stop")
        .then(() => {
